(function () {
  ...
  var events = (
    "mousemove mouseover mouseout mousedown mouseup click touchstart " +
    "dblclick focus blur submit change"
  ).split(" ");
  ...
  Util.fullScreen = function () {
    if (
      ...
      document.body.style.height =
        window.outerHeight / window.devicePixelRatio + 1 + "px";
      setTimeout(function () {
        window.scrollTo(1, 1);
      }, 0);
    } else {
...
(function () {
  ...
  function check_position(f1, f2) {
    ...
    if (
      ...
      return [
        f1.col > f2.col ? -abs(diff_x) + w : abs(diff_x) - w,
        f2.ty - f1.ty
      ];
    } else if (
      ...
      return [
        f2.tx - f1.tx,
        f1.row > f2.row ? -abs(diff_y) + h : abs(diff_y) - h
      ];
    }
  }
  var Piece = Cevent.Shape.extend({
    ...
    draw: function (ctx) {
      ...
      var x = this.x - this.width / 2 - 0.5;
      var y = this.y - this.height / 2 - 0.5;
      if (isDesktop) {
    ...
    hitTest: function (point) {
      ...
      return ctx.isPointInPath(
        point.x * getPixelRatio(),
        point.y * getPixelRatio()
      );
    }
  }),
  ...
  jigsaw.Jigsaw = Class.extend({
    init: function (opts) {
      ...
      this.ce.beforeDraw(function () {
        if (showPreview)
          self.ce.ctx.drawImage(
            self.preview,
            self.ce.cv.width / 2 - self.preview.width / 2,
            0
          );
      });
      ...
      eventBus.on(
        jigsaw.Events.PARTS_NUMBER_CHANGED,
        this.set_parts.bind(this)
      );
    ...
    render: function () {
      ...
      offset = ~~(
        (document.documentElement.clientWidth / 2) * getPixelRatio() -
        (width * cols) / 2
      );
      this.clear();
        ...
        if (i < cols && flag == this.flag) {
          if (j < rows) {
            ...
            this.ce
              .piece(
                width * i,
                height * j + this.opts.offsetTop,
                window.G_vmlCanvasManager ? this.tmp_img : this.img,
                width,
                height,
              ...
              .attr({
                ...
                row: j,
                offset: offset,
                stroke: this.opts.border ? "red" : ""
              })
              .get(-1)
              .render(ox, oy - this.opts.offsetTop);
            if (!this.opts.shuffled) {
              this.ce.redraw();
            }
  ...
  function jigsaw_events(ce, eventBus, rotate) {
    ce.drag("*", {
      ...
      afterMove: function (c, e) {
        ...
        if (
          !ce.getAll("piece").length &&
          ce.getAll("group").length == 1 &&
          ce.shuffled
        ) {
          ce.shuffled = false;
...
(function () {
  ...
  jigsaw.UI = Class.extend({
    init: function (eventBus, parts) {
      ...
      eventBus.on(
        jigsaw.Events.SHOW_FILEPICKER,
        this.show_filepicker.bind(this)
      );
    },
    stop_clock: function () {
      uuid++;
